...
#git -C /panda submodule update --init dtc && 
RUN git -C /panda rev-parse HEAD > /usr/local/panda_commit_hash && \
    mkdir -p /panda/build && cd /panda/build && \
...
RUN PRETEND_VERSION=$(cat /tmp/savedversion) ninja -C /panda/build -j "$(nproc)" || true 

RUN PRETEND_VERSION=$(cat /tmp/savedversion) ninja -C /panda/build -j "$(nproc)"
