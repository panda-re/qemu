...
static void post_reset(qemu_plugin_id_t id) {
    fprintf(stderr, "syscalls: Fatal error: capstone is unavailable.\n");
    fprintf(stderr, "syscalls: rebuild qemu with --enable-capstone\n");

    // If a user specifies an invalid plugin, should we just warn or raise a fatal error?
    //exit(1);
}
...
static void vcpu_tb_trans(qemu_plugin_id_t id, struct qemu_plugin_tb *tb) {
  ...
  if (insn_disas[0] == '\0') {
    // If capstone is disabled, we'll get an empty string back for disassembly.
    // Otherwise we'll always get something (even if capstone fails??)
    qemu_plugin_reset(plugin_id, post_reset);
  } else if (is_syscall_fn(insn_disas)) {
