...
#define READ_INFO_X(key_file_get, ki, memb, gerr, errcount, errbmp)\
	...
	if (gerr != NULL) { errcount++; g_error_free(gerr); gerr = NULL; fprintf(stderr, "failed to read " #memb); }\
...
#define OPTIONAL_READ_INFO_X(key_file_get, ki, memb, gerr, errcount, errbmp)\
	((ki)->memb) = key_file_get(keyfile, group_real, #memb, &gerr);\
	if (gerr != NULL) { g_error_free(gerr); gerr = NULL; fprintf(stderr, "failed to read " #memb); ((ki)->memb) = (uint64_t)NULL;}\
...
int read_kernelinfo(gchar const *file, gchar const *group, struct kernelinfo *ki) {
	...
	/* check number of errors */
	{
		...
		if (nerrors > 0) {
			fprintf(stderr, "%d errors reading from group %s", nerrors, group_real);
			rval = -1;
		}
	}
