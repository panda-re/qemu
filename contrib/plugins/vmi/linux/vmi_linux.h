/*!
 * This file is heavily based on PANDA.re's vmi Linux plugin's vmi_linux.h file.
 * The offset getter macros have been based off the code from
 * linux_vmi plugin and TEMU's read_linux.
 *
 * @author Andrew Fasano <fasano@mit.edu>
...
/**
 * Stupid panda shim
 * Reads memory from a given virtual address into a provided buffer.
 *
 * @param gva The virtual address to read from.
 * @param buf A pointer to the buffer where the data will be stored.
 * @param length The number of bytes to read.
 * @param is_write A flag indicating if the operation is a write (not implemented).
...
static int panda_virtual_memory_rw(uint64_t gva, void* buf, size_t length, bool is_write) {
    ...
    // Create a GByteArray with preallocated size to avoid reallocations
    GByteArray *data = g_byte_array_sized_new(length);
    g_byte_array_set_size(data, length);

    // Read memory from the virtual address into the GByteArray
    bool success = qemu_plugin_read_memory_vaddr(gva, data, length);
    ...
    // Copy the data from the GByteArray into the provided buffer
...
static inline int _funcName(target_ptr_t _paramName, _retType* _retName) { \
    size_t ret_size = ((_retSize) == OG_AUTvmiZE) ? sizeof(_retType) : (_retSize); \
    OG_printf(#_funcName ":1:%lx:%d\n", _paramName, _offset); \
    OG_printf(#_funcName ":2:%lx:%zu\n", _paramName + _offset, ret_size); \
 ...
 */
IMPLEMENT_OFFSET_GETN(get_vfsmount_parent, vfsmount, target_ptr_t, vfsmount_parent, OG_AUTvmiZE, ki.path.mnt_parent_offset)

/**
 * @brief Retrieves the dentry struct associated with a vfsmount struct.
 */
IMPLEMENT_OFFSET_GETN(get_vfsmount_dentry, vfsmount, target_ptr_t, vfsmount_dentry, OG_AUTvmiZE, ki.path.mnt_mountpoint_offset)

/**
 * @brief Retrieves the mnt_root dentry struct associated with a vfsmount struct.
 */
IMPLEMENT_OFFSET_GETN(get_vfsmount_root_dentry, vfsmount, target_ptr_t, root_dentry, OG_AUTvmiZE, ki.path.mnt_root_offset)
 ...
 */
IMPLEMENT_OFFSET_GET2LN(get_dentry_dname, dentry, target_ptr_t, dname_funcp, OG_AUTvmiZE, ki.path.d_op_offset, ki.path.d_dname_offset)

/**
 * @brief Retrieves the parent of a dentry.
 */
IMPLEMENT_OFFSET_GETN(get_dentry_parent, dentry, target_ptr_t, dentry_parent, OG_AUTvmiZE, ki.path.d_parent_offset)
...
static inline char *read_dentry_name(target_ptr_t dentry) {
    ...
    while (current_dentry_parent != current_dentry) {
        ...
        if (pcomp_length == (uint32_t)-1) { // Not sure why this happens, but it does
              printf("Warning: vmi_linux Unhandled pcomp value, ignoring\n");
              break;
        }
        ...
        if (pcomp_capacity < pcomp_length) {
            ...
            if (pcomp == NULL) {
              printf("Warning: vmi_linux pcomp g_realloc failed\n");
              break;
            }
        }
...
void fill_vmithread(VmiThread *t, target_ptr_t task_addr);
